...
export function constructApi(scope: Construct, props: ApiProps) {
  ...
  const handler = new lambda.Function(scope, "GetImageLambda", {
    ...
    code: lambda.Code.fromAsset(getLambdaPath("get_image_lambda")),
  ...
  const getOrSetMetadataHandler = new lambda.Function(
    scope,
    "GetOrSetMetadataLambda",
    {
      functionName: "GetOrSetMetadataLambda",
      code: lambda.Code.fromAsset(getLambdaPath("get_or_set_reaction_lambda")),
      runtime: lambda.Runtime.PROVIDED_AL2023,
      architecture: lambda.Architecture.ARM_64,
      handler: "not.required",
      environment: {
  ...
  getOrSetMetadataHandler.addToRolePolicy(
    new PolicyStatement({
      actions: ["dynamodb:*"],
      resources: ["*"],
    }),
  ...
  const setFavoriteRecentHandler = new lambda.Function(
    scope,
    "SetFavoriteRecentLambda",
    {
      functionName: "SetFavoriteRecentLambda",
      code: lambda.Code.fromAsset(getLambdaPath("set_favorite_recent_lambda")),
      runtime: lambda.Runtime.PROVIDED_AL2023,
      architecture: lambda.Architecture.ARM_64,
      handler: "not.required",
      environment: {
  ...
  setFavoriteRecentHandler.addToRolePolicy(
    new PolicyStatement({
      actions: ["dynamodb:*"],
      resources: ["*"],
    }),
  ...
  todaysMetadata.addMethod(
    "GET",
    new apiGateway.LambdaIntegration(getOrSetMetadataHandler),
  );
  todaysMetadata.addMethod(
    "PUT",
    new apiGateway.LambdaIntegration(getOrSetMetadataHandler),
  ...
  setFavorite.addMethod(
    "PUT",
    new apiGateway.LambdaIntegration(setFavoriteRecentHandler),
  );
}
...
export function constructEvents(scope: Construct, props: EventProps) {
  const dailySetupHandler = new lambda.Function(scope, "DailySetupLambda", {
    ...
    code: lambda.Code.fromAsset(getLambdaPath("daily_setup_lambda")),
    runtime: lambda.Runtime.PROVIDED_AL2023,
    architecture: lambda.Architecture.ARM_64,
    handler: "not.required",
    environment: {
  ...
  dailySetupHandler.addToRolePolicy(
    new PolicyStatement({
      actions: ["s3:*"],
      resources: ["*"],
    }),
  ...
  dailyEvent.addTarget(
    new targets.LambdaFunction(dailySetupHandler, {
